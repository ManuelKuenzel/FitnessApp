...
def start_training():
    level = request.form.get('level', 'einfach')  # Default ist 'einfach'
    exercises_data = load_exercises()
    filtered_exercises = [ex for ex in exercises_data if ex['level'] == level]
    
    # Hier könntest du noch die Logik zur Verarbeitung der ausgewählten Übungen hinzufügen
    return render_template('countdown.html', exercises=filtered_exercises)
